...
| **Plan 额度监控** | 真实套餐档位、Credit 余量、剩余百分比、到期时间（经控制台接口获取，自动续期） |
| **凭据自动续期** | access 2 小时失效后自动用 29 天有效的 refresh 换新，无需人工干预 |
...
> 只有查额度才用这套凭据。
>
> **凭据会自动续期，只需 29 天登录一次。** Cookie 里的 `Oasis-Token` 其实是
> 两段 JWT 拼起来的：
>
> ```
...
> | 段 | 寿命 | 说明 |
> | --- | --- | --- |
> | access | **2 小时**（`exp - create_at` 恒为 7200 秒） | 调额度接口用 |
> | refresh | **29 天** | 用来换新的 access |
>
> access 一旦失效，本服务会自动调
> `POST /passport/proto.api.passport.v1.PassportService/RefreshToken`（空 body，
> 带完整的两段凭据）换回一组新的 access + refresh，把新 Cookie 加密存回库里。
> 实测 **access 过期后仍可续期**，续期后的新凭据能继续正常查询，因此额度监控
> 可以长期跑下去。
>
> **⚠️ 别信 Cookie 自带的过期时间**（写着 2027 年）——真实寿命以上面两段 JWT
> 的 `exp` 为准。界面上会显示 access 与 refresh 各自还剩多久。
>
> 唯一需要人工介入的情况：超过 29 天没访问（refresh 也过期），或主动退出登录。
> 届时重新复制一次凭据即可。
>
